...
class LateFusionPSPNet(nn.Module):
    ...
    def forward(self, rgb, ms):
        ...
        out = out_rgb + out_ms   # late fusion
...
if __name__ == "__main__":
    ...
    for run in range(NUM_RUNS):
        ...
        model = LateFusionPSPNet(ms_channels=6).to(device)

        optimizer = torch.optim.Adam(model.parameters(), lr=1e-4)
        ce_loss = nn.CrossEntropyLoss()

        best_iou = 0
        best_path = f"best_pspnet_run{run}.pth"
    ...
    # SAVE CSV
    # -----------------------------
    with open("pspnet_5runs_results.csv","w",newline="") as f:
        writer = csv.writer(f)
