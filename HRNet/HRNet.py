...
class WeedyRiceDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(rgb, dtype=torch.float32), torch.tensor(ms, dtype=torch.float32), torch.tensor(mask)
...
class HRNetBackbone(nn.Module):
    def __init__(self, in_chans):
        ...
        with torch.no_grad():
            ...
            feats = self.net(dummy)
            self.out_channels = feats[-1].shape[1]
...
# Model (NO CBAM)
# -----------------------------
class LateFusionHRNet(nn.Module):
    def __init__(self, ms_channels=6):
        super().__init__()

        self.rgb_net = HRNetBackbone(3)
        self.ms_net = HRNetBackbone(ms_channels)

        feat_ch = self.rgb_net.out_channels
        self.fusion_conv = nn.Conv2d(feat_ch * 2, 256, 1)
    ...
    def forward(self, rgb, ms):
        ...
        feat_ms  = self.ms_net(ms)

        if feat_rgb.shape[-2:] != feat_ms.shape[-2:]:
            feat_ms = F.interpolate(feat_ms, size=feat_rgb.shape[-2:], mode='bilinear', align_corners=False)
...
print("\n🎨 Generating HRNet Sample Predictions...\n")

# load one trained model
model.load_state_dict(torch.load("best_hrnet_run0.pth"))
...
# optional save
plt.savefig("hrnet_predictions.png", dpi=300, bbox_inches='tight')
...
for metric in results[0].keys():
    ...
    print(f"{metric}: Mean={np.mean(vals):.4f}, Std={np.std(vals):.4f}, Var={np.var(vals):.6f}")